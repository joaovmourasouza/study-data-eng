...
class CommonStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, enviroment: EnviromentEnum, **kwargs) -> None:
        ...
        for subnet in self.vpc.private_subnets:
            self.orders_rds_sg.add_ingress_rule(peer=ec2.Peer.ipv4(subnet.cidr_block), connection=ec2.Port.tcp(5432))

        self.orders_rds_parameter_group = rds.ParameterGroup(
            self,
            f"orders-rds-parameter-group-{self.enviroment.value}",
            engine=rds.Engine.postgres(version=rds.PostgresEngineVersion.VER_15_1),
            parameters={
                "max_connections": "100",
                "shared_buffers": "25%",
                "work_mem": "64MB",
